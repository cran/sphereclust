mixsespc.contour <- function(probs, mu, theta, bgcol = "snow", dat = NULL,
  ...
  mat <- sphereclust::dmixsespc(wa, probs, mu, theta)

  ## Bug 1 fix: use n1 (rows = x1) and n2 (cols = x2) explicitly
  mat <- matrix(mat, nrow = n1, ncol = n2, byrow = TRUE)

  ## Bug 3 fix: use quantile-based levels so contours span meaningful density
  ...
  ## Bug 2 fix: push lines slightly outside sphere (radius 1.002) to avoid z-fighting
