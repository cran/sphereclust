mixkent.contour <- function(probs, G, param, bgcol = "snow", dat = NULL,
                             col = NULL, lat = 50, long = 50) {

  g <- length(G)
  m1 <- matrix(nrow = g, ncol = 3)
  for ( i in 1:g )  m1[i, ] <- G[[ i ]][, 1]
  m  <- Directional::euclid.inv(m1)

  x1 <- seq( min(m[, 1]) - lat,  max(m[, 1]) + lat,  by = 0.1 )
  x2 <- seq( min(m[, 2]) - long, max(m[, 2]) + long, by = 0.1 )

  ## Bug 1 fix: clip to valid lat/lon range
  x1 <- x1[ x1 >= -90  & x1 <= 90  ]
  x2 <- x2[ x2 >= -180 & x2 <= 180 ]
  ...
  mat <- sphereclust::dmixkent(wa, probs, G, param)

  ## Bug 1 fix: explicit nrow and ncol
  mat <- matrix(mat, nrow = n1, ncol = n2, byrow = TRUE)

  ## Bug 3 fix: quantile-based levels instead of linear nlevels
  finite_vals <- mat[ is.finite(mat) ]
  levs <- quantile(finite_vals, probs = seq(0.5, 0.97, length.out = 10))
  levs <- unique(levs)
  ...
  ## Bug 2 fix: scale to radius 1.002 to avoid z-fighting with sphere surface
